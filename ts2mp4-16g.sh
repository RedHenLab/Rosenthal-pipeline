...
# Changelog:
#
#	2016-02-25 Terminate node when done compressing
#	2016-02-17 Groeling version for Archive files only
#	2016-02-12 Complete processing for _Archive_, _BR_, and _PT_ files (provisional)
...
#	2013-09-14 Add silent lock in HOME -- the POND lock sometimes fails
...
if [ "$1" = "-h" -o "$1" = "--help" -o "$1" = "help" ]
 then echo -e "\n$SCRIPT"
  ...
  echo -e "\tThe script is designed to run on groeling@hoffman2.\n"
   exit
fi
...
if [ -z "$4" ]
  ...
  # Local processing -- local source but shared scratch
    ...
    # Drops
    S2DIR=/u/scratch/g/groeling/drops
...
# Short forms
FIL="${1##*/}" EXT="${FIL##*.}"  FIL="${FIL%.*}"  SFIL=$WORK/$FIL

# Prefer ts, then mpg, then mov (for mp4 to mp4 conversions)
if [ -f $WORK/$FIL.ts ] ; then EXT=ts
  elif [ -f $WORK/$FIL.mpg ] ; then EXT=mpg
  elif [ -f $WORK/$FIL.mov ] ; then EXT=mov
  else echo -e "\n\tUnable to locate the file $FIL\n" ; exit
fi

# Copy the files from the scratch spond if present -- used for interrupted jobs
#if [ ! -f $WORK/$FIL.mpg ] ; then if [ -f $S2DIR/$FIL.mpg ] ; then mv $S2DIR/$FIL.* $WORK/ ; fi ; fi

# Verify the file exists
if [ ! -s $SFIL.$EXT ] ; then echo -e "\n\tNot finding a file called $1 in the pond\n" ; exit ; fi
...
if [[ -d $HOME/tv2/pond/$FIL.mp4.done || -d $DAYS/$DDIR/$FIL.mp4.done || -d $R2DIR/$FIL.mp4.done ]] ; then echo -e "\n\tMarked as done -- $FIL.mp4.done\n" ; exit ; fi
...
if [ -f /work/pond/$FIL.txt ] ; then TXT=/work/pond/$FIL.txt
  elif [ -f /work/day/$DDIR/$FIL.txt3 ] ; then TXT=/work/day/$DDIR/$FIL.txt3
  elif [ -f /work/day/$DDIR/$FIL.txt1 ] ; then TXT=/work/day/$DDIR/$FIL.txt1
  elif [ -f $HOME/tv2/day/$DDIR/$FIL.txt3 ] ; then TXT=$HOME/tv2/day/$DDIR/$FIL.txt3
fi

# Get the show duration from the text file
if [ -z "$LEN" ] ; then
  DUR="$( grep -a '^DUR|' $TXT | tail -n1 | cut -d"|" -f2 )"
...
if [ "$( mkdir $HOME/tv2/pond/$FIL.compressed 2> /dev/null; echo $? )" = "0" ]
  then echo -e "`date +%F\ %H:%M:%S` \t${SCRIPT%.*} \t$QNUM\t$HOST \t$DUR \t$FIL.$EXT" > $HOME/tv2/pond/$FIL.compressed/$QNUM
...
# Ensure there's a node reservation as a backup
...
# Ensure the file is reserved (the reservation may have been deleted by the node-daemon) -- you should check this belongs to the current node
...
# Ensure the file is reserved for fetching (check local $FIL.fetched first!)
...
if [ ! -s $SFIL.mp4 ] ; then
  ...
  # Allow $MAXDROP seconds of slack in video and audio length, and rename videos that are too long or too short
  if [[ ( "${DIFFVID%.*}" -lt 0 && "$( echo "$DIFFVID < -$MAXDROP" | bc )" = 1 ) || ( "${DIFFAUD%.*}" -lt 0 && "$( echo "$DIFFAUD < -$MAXDROP" | bc )" = 1 ) ]]
    then echo -e "`date +%F\ %H:%M:%S` \t${SCRIPT%.*} \t$QNUM\t$HOST \tFfmpeg    \tDROP "$DIFFVID"s \t$FIL.mp4" | tee -a $LOGS/reservations.$( date +%F )
      if [[ -s $SFIL.mp4 && ( "${DIFFVID%.*}" -gt -2000 || "${DIFFAUD%.*}" -gt -2000 ) ]] ; then mv $SFIL.mp4 $SFIL-VDROP$DIFFVID-ADROP$DIFFAUD.mp4 ; else rm -r $SFIL.mp4 ; fi
     ...
     then echo -e "`date +%F\ %H:%M:%S` \t${SCRIPT%.*} \t$QNUM\t$HOST \tFfmpeg    \tADDS "$DIFFVID"s \t$FIL.mp4" | tee -a $LOGS/reservations.$( date +%F )
      if [[ -s $SFIL.mp4 && ( "${DIFFVID%.*}" -lt 2000 ) || ( "${DIFFAUD%.*}" -lt 2000 ) ]] ; then mv $SFIL.mp4 $SFIL-VADDS$DIFFVID-AADDS$DIFFAUD.mp4 ; else rm -r $SFIL.mp4 ; fi
...
if false ; then LOOP=3
  ...
  if [[ ( "${DIFFVID%.*}" -lt 0 && "$( echo "$DIFFVID < -$MAXDROP" | bc )" = 1 ) || ( "${DIFFAUD%.*}" -lt 0 && "$( echo "$DIFFAUD < -$MAXDROP" | bc )" = 1 ) ]]
    then echo -e "`date +%F\ %H:%M:%S` \t${SCRIPT%.*} \t$QNUM\t$HOST \tHandbrake  \tDROP "$DIFFVID"s \t$FIL.mp4" | tee -a $LOGS/reservations.$( date +%F )
      if [[ -s $SFIL.mp4 && ( "${DIFFVID%.*}" -gt -200 || "${DIFFAUD%.*}" -gt -200 ) ]] ; then mv $SFIL.mp4 $DROPS/$FIL-VDROP$DIFFVID-ADROP$DIFFAUD.mp4 ; else rm -r $SFIL.mp4
     ...
     then echo -e "`date +%F\ %H:%M:%S` \t${SCRIPT%.*} \t$QNUM\t$HOST \tHandbrake  \tADDS "$DIFFVID"s \t$FIL.mp4" | tee -a $LOGS/reservations.$( date +%F )
      if [[ -s $SFIL.mp4 && ( "${DIFFVID%.*}" -lt 200 ) || ( "${DIFFAUD%.*}" -lt 200 ) ]] ; then mv $SFIL.mp4 $DROPS/$FIL-VADDS$DIFFVID-AADDS$DIFFAUD.mp4 ; else rm -r $SFIL.mp4 ; fi
...
# Receipt
echo -e "`date +%F\ %H:%M:%S` \t${SCRIPT%.*} \t$QNUM\t$HOST \tCmprss time \t$PTIM     \t$FIL" | tee -a $LOGS/reservations.$( date +%F )

set -xv
# Confirm completion and copy the file to storage
if [ -s $SFIL*.mp4 ]
  then mv /work/pond/$FIL.compressed /work/pond/$FIL.mp4.done
    COPY1="$( $LBIN/copy2csa-02.sh $SFIL*.mp4 | tail -n2 | grep OK )"
    if [[ $COPY1 != OK* ]] ; then COPY1="$( $LBIN/copy2csa-00.sh $SFIL*.mp4 | tail -n2 | grep OK )" ; fi
  else COPY1=""
fi

# Keep the .txt3 file
cp -nrp /work/day/$DDIR/$FIL.txt3 $DAYS/$DDIR/

# Get the start of processing from the logs -- this starts with the .len file download
#DAY=$( date +%F ) YAY=$( date -d "-1 day" +%F )
#PSTART="$( date -d "`grep -m 1 -h $FIL $LOGS/reservations.{$YAY,$DAY} | head -n 1 | cut -d" " -f1-2`" +%s )"

# Get the start of processing from the local reservation -- this starts with the .mpg file download
PSTART=0 ; while [ "$PSTART" -eq 0 ] ; do PSTART="$( date -r /work/pond/$FIL.reserved +%s )" ; done

# Convert to hours mins secs
PTIME="$(date -ud "+$[`date +%s`-PSTART] seconds"\ $(date +%F) +%H:%M:%S)"

# Completion time
if [ -d $SFIL.mp4.done ] ; then

  RHOST="$( cut -d" " -f1 $HOME/tv2/pond/$FIL.len )"
  echo -e "`date +%F\ %H:%M:%S` \t${SCRIPT%.*} \t$QNUM\t$HOST \tCompleted ${RHOST^^} \t$PTIME * \t$FIL" | tee -a $LOGS/reservations.$( date +%F )

  # Keep the .len, the .txt3, and the compression confirmation files
  cp -nrp $HOME/tv2/pond/$FIL.len /work/day/$DDIR/$FIL.txt3 /work/pond/$FIL.mp4.done $DAYS/$DDIR/

fi

# Rename or remove the primary and secondary reservations, clean up
if [[ $COPY1 == OK* ]]
  then echo -e "`date +%F\ %H:%M:%S` \t${SCRIPT%.*} \t$QNUM\t$HOST \tTransfer  \tSuccess   \t$FIL.mp4" | tee -a $LOGS/reservations.$( date +%F )
    rm -rf $WORK/$FIL.* /work/day/$DDIR/$FIL.* $HOME/tv2/pond/$FIL.* $R2DIR/$FIL.* #; qdel $QNUM
  # As a safety fallback, if copying fails, move any processed files to the central pond (should be very unusual and maybe not even a good idea)
  else echo -e "`date +%F\ %H:%M:%S` \t${SCRIPT%.*} \t$QNUM\t$HOST \tTransfer  \tFAILED    \t$FIL.mp4" | tee -a $LOGS/reservations.$( date +%F )
    cp -p /work/pond/$FIL.mp4 /work/day/$DDIR/$FIL.txt3 /work/day/$DDIR/$FIL.srt /work/day/$DDIR/$FIL.ccx.bin $HOME/tv2/pond/
    rm -rf $WORK/$FIL.* /work/day/$DDIR/$FIL.* $R2DIR/$FIL.* #; qdel $QNUM
fi
